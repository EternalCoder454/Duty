...
import net.dutymod.server.net.util.QuietDecoderException;
...
import static net.dutymod.server.net.util.WellKnownExceptions.BAD_LENGTH_CACHED;
import static net.dutymod.server.net.util.WellKnownExceptions.VARINT_BIG_CACHED;
    ...
    public void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        if (length > 0) {
            if (in.readableBytes() < length) {
                ...
                // Vanilla's decode reports every framed packet to the bandwidth monitor, so this
                // overwrite has to as well. Upstream made the call optional and defaulted it off,
                // which leaves the F3 bandwidth readout silently empty; the monitor is null unless
                // bandwidth debugging is switched on, so honouring it costs one null check.
