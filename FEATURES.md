...
## The loader axis, done for every module

All five feature modules are now split the way the framework is: `src/main` names no loader,
`src/neoforge` holds everything that does. The convention lives once in the root build rather than
six times.

| Module | Loader-neutral | Loader-specific |
| --- | ---: | ---: |
| duty-framework | 6 | 2 |
| duty-memory | 38 | 2 |
| duty-client | 216 | 8 |
| duty-fixerupper | 159 | 20 |
| duty-server | 63 | 2 |
| duty-essentials | 66 | 2 |

**36 of 574 files touch a loader.** That is the useful number: Duty was already almost entirely
loader-neutral, and the work was finding the 6% rather than rewriting the 94%.

### Where the loader actually leaked

Three shapes, and only the first is what an import scan finds.

* **Direct API use** -- entry points, event bus registration, NeoForge's model pipeline
  (`ModelData`, `UnbakedModelParser`), capabilities, registry events. Moved as-is. NeoForge's model
  API has no Fabric equivalent, so those mixins are permanently loader-specific rather than
  awaiting a port.
* **A shared class with one loader-specific method.** `Quiet` holds keybinds, music state and a
  per-tick check that eighteen loader-neutral mixins depend on, and one `register(IEventBus)`. The
  class went back to `src/main` and only the registration moved, as `QuietNeoForge`.
* **A flag only the loader can set.** Two loader-neutral FixerUpper mixins read
  `FixerUpperForge.registryEventsFired`. No import of NeoForge, no way for a scan to see it, and it
  would have broken a Fabric build with a missing class. It now lives in `FixerUpperState`, which
  the entry point writes -- dependencies point one way, loader code to shared code, never back.

`QuietMixinConfig` needed NeoForge only for `FMLPaths.CONFIGDIR`; it reads
`Platform.get().configDir()` now and is loader-neutral.

### The split broke two checkers, silently

Both worth recording, because both are the failure this project keeps guarding against.

* **The checkers only scanned `src/main/java`.** After the move, `check-shadows` reported 162
  instead of 166 and passed. It was not verifying the moved files at all. All five now walk every
  source set.
* **Fourteen FixerUpper mixins stopped applying.** Its config is generated by an annotation
  processor that ran over `src/main` only, so the moved mixins fell out: 114 entries became 100.
  The classes still shipped and the build was green. `check-mixin-configs` caught it only because
  the *count* changed -- it verifies that listed entries resolve, not that shipped mixins are
  listed. The processor now runs over both source sets into two configs, and
  **`tools/check-orphan-mixins.py`** was added to check the direction that was missing.

Two configs rather than one merged is deliberate: it is what lets a Fabric build ship
`duty_fixerupper.mixins.json` without `duty_fixerupper_neoforge.mixins.json`.

## The version axis: what it needs, per feature

The loader axis is done. The version axis is not, and it is not a build-system problem -- some
features cannot exist below a given version because the class they optimise does not exist yet.

| Depends on | Since | Where | Consequence |
| --- | --- | --- | --- |
| Data components | 1.20.5 | duty-memory's `PatchedDataComponentMap` work, 2 FixerUpper mixins | Cannot exist on 1.20.1 in any form |
| Recipe display system | 1.21.2 | duty-client's ClientCrafting | Cannot exist on 1.20.1 or 1.21.1 |
| `RenderType`/`RenderSetup` rework | 26.1 | 7 duty-client batching files | Needs a rewrite per version, not a rename |
...
| `Identifier` | 26.1 | 62 files across four modules | A rename of `ResourceLocation`; mechanical |

The last three rows are ordinary porting work. The first three are not: on 1.20.1, duty-memory's
central feature and duty-client's crafting prediction have nothing to attach to, so a 1.20.1 build
ships those modules with those features absent rather than reimplemented.

**What reaching the other three targets still needs**, beyond writing the code:

1. A version-gating mechanism -- Stonecutter or per-version source sets -- since the same file must
   compile against `ResourceLocation` on one target and `Identifier` on another.
2. Three more toolchains and their Minecraft artifacts.
3. A checker run per target. Every checker here compares against one Minecraft jar; `check-shadows`
