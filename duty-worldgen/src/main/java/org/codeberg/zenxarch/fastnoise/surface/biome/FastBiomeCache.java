...
public final class FastBiomeCache {
  ...
  public FastBiomeCache(Holder<Biome>[] biomes, final ChunkAccess[] chunks) {
    this.chunks = chunks;
    this.biomes = biomes;
  ...
  public Holder<Biome> getBiomeOrNull(int blockX, int blockY, int blockZ) {
  ...
  private Holder<Biome> getBiomeOrNull(int minBiomeY, int bitIdx) {
  ...
  private int[] getCacheForY(int by) {
    final var cby = Mth.clamp(by, minYInBiomes, maxYInBiomes);
    ...
    if (cached == null) {
      return (this.cache[secY] =
          new FastChunkBiomeCache(
              biomes,
              this.chunks[0].getSections()[secY],
              this.chunks[1].getSections()[secY],
              this.chunks[2].getSections()[secY],
              this.chunks[3].getSections()[secY],
              this.chunks[4].getSections()[secY],
              this.chunks[5].getSections()[secY],
              this.chunks[6].getSections()[secY],
              this.chunks[7].getSections()[secY],
              this.chunks[8].getSections()[secY]));
    }
    return cached;
  }
}
