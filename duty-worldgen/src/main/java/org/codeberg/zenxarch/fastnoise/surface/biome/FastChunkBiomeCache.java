...
public final class FastChunkBiomeCache {
  ...
  public FastChunkBiomeCache(
      Holder<Biome>[] biomes,
      LevelChunkSection aa,
      LevelChunkSection ab,
      LevelChunkSection ac,
      LevelChunkSection ba,
      LevelChunkSection bb,
      LevelChunkSection bc,
      LevelChunkSection ca,
      LevelChunkSection cb,
      LevelChunkSection cc) {
  ...
  private void fillArray(
      int[] cache, int y, Holder<Biome>[] biomes, LevelChunkSection[] sections) {
    ...
    int singleBiome = -1;
    if (((PalettedContainer<Holder<Biome>>) sections[4].biomes).data.palette()
        instanceof SingleValuePalette<Holder<Biome>> single) {
      singleBiome = indexOf(biomes, single.value);
    }

    for (int z = 0; z < 4; z++) {
      for (int x = 0; x < 4; x++) {
        localBiomes[z + 1][x + 1] =
            singleBiome == -1 ? indexOf(biomes, sections[4].getNoiseBiome(x, y, z)) : singleBiome;
      }
    }
  ...
  private int indexOf(Holder<Biome>[] biomes, Holder<Biome> biome) {
  ...
  private Holder<Biome> getBiome(LevelChunkSection[] sections, int x, int y, int z) {
    int cx = (x >> 2) + 1;
    int cz = (z >> 2) + 1;
    ...
    return section.getNoiseBiome(lx, y, lz);
  }
}
