...
public class FramebufferBlitter {
    ...
    public static void blit(RenderTarget source, GuiGraphicsExtractor guiGraphics) {
        if (Gnetum.config.downscale.get()) {
            ...
            ((Gui_Graphics_Accessor)guiGraphics).getGuiRenderState().addGuiElement(new FramebufferRenderState(source));
        }
    }
