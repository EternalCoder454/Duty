...
public abstract class ItemTransformMixin {
    ...
    @Unique private @Nullable Quaternionf duty$quatNormal;
    @Unique private @Nullable Quaternionf duty$quatFlipped;
    ...
    public void apply(boolean flag, PoseStack.Pose pose) {
        ...
        Quaternionf q = flag ? this.duty$quatFlipped : this.duty$quatNormal;
        if (q == null) {
            ...
            if (flag) {
                this.duty$quatFlipped = q;
            } else {
                this.duty$quatNormal = q;
            }
        }
        pose.rotate(q);
