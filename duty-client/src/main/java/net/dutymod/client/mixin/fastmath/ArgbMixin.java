package net.dutymod.client.mixin.fastmath;
...
public class ArgbMixin {
    ...
    @Unique
    private static final int[] duty$RECIPROCALS = new int[256];
    @Unique
    private static final float[] duty$FROM_8_BIT = new float[256];

    static {
        for (int i = 0; i < 256; ++i) {
            duty$FROM_8_BIT[i] = (float) i / 255.0F;
        }
        duty$RECIPROCALS[0] = 0;
        for (int i = 1; i < 256; i++) {
            int min_m = 0;
    ...
    public static float redFloat(int i) {
        return duty$FROM_8_BIT[(i >> 16) & 255];
    }
    ...
    public static float greenFloat(int i) {
        return duty$FROM_8_BIT[(i >> 8) & 255];
    }
    ...
    public static int alphaBlend(int bottomColor, int topColor) {
        ...
        int reciprocal = duty$RECIPROCALS[outA];
