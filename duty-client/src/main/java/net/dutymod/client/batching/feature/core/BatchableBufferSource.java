...
public class BatchableBufferSource extends MultiBufferSource.BufferSource implements AutoCloseable {
    ...
    public VertexConsumer getBuffer(final RenderType renderType) {
        ...
        if (hasBufferForRenderType) {
            if ((Batching.config.debug_only_use_last_usage_for_batch_ordering || renderType.name.contains("immediatelyfast:renderlast")) && this.activeRenderTypes.contains(renderType)) { // Fix for https://github.com/RaphiMC/Batching/issues/181
