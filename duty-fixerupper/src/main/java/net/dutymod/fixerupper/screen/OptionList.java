...
public class OptionList extends ContainerObjectSelectionList<OptionList.Entry> {
    ...
    private static final Component OPTION_ON = Component.translatable("duty.option.on").withStyle(style -> style.withColor(ChatFormatting.GREEN));
    private static final Component OPTION_OFF = Component.translatable("duty.option.off").withStyle(style -> style.withColor(ChatFormatting.RED));
    ...
    class OptionEntry extends Entry {
        ...
        public OptionEntry(String optionName, Option<?> option) {
            ...
            if(this.option.isModDefined()) {
                ...
                toggleTooltip = Tooltip.create(Component.translatable("duty.option." + (this.option.getValue() ? "enabled" : "disabled"))
                        .append(Component.translatable("duty.option.mod_override", disablingMods)));
            }
        ...
        public void extractContent(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, boolean isHovering, float partialTicks) {
            ...
            if(this.option.isUserDefined())
                nameComponent = nameComponent.withStyle(style -> style.withItalic(true)).append(Component.translatable("duty.config.not_default"));
