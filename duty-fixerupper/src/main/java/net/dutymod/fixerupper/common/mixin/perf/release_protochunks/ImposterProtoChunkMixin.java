...
public class ImposterProtoChunkMixin {
    ...
    private BlockEntity avoidLeakingLiveBE(BlockEntity original, @Local(ordinal = 0, argsOnly = true) BlockPos pos) {
        if (!this.allowWrites && original != null && original.getLevel() != null) {
            FixerUpper.LOGGER.debug("Blocked accessing the main level BlockEntity at {} from the ImposterProtoChunk wrapper, as this is unsafe during worldgen.", pos, new Exception("Stacktrace"));
            return null;
        } else {
            return original;
        }
